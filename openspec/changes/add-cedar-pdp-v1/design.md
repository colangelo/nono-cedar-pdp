...
**D6 — `args` is a Cedar `Set<String>`.** Sets have no index access, so positional argument policy is unexpressible rather than merely discouraged — which is the correct response to upstream's lossy argv. `argv` (joined) and `arg_count` cover the remaining legitimate cases. Consequence documented as a policy-authoring rule: `argv like "*--force*"` also matches text inside a single quoted argument, so `argv` globs are safe in `forbid` and unsound in `permit`. Confirmed empirically: `git commit -m "do not --force this"` matches.
...
- **`argv` glob over-matching producing spurious denials** → confined to `forbid` policies where over-matching is fail-safe, and paired with the `chain` fallback so a spurious denial becomes a prompt rather than a hard block.
