...
- [x] 12.3 Narrow the loader lint that fired on the fix: the flattening hazard is *searching* a joined string, so flag only a `permit` whose `argv_tail` test is not a positional pin (an anchored `like` pattern or an `==` is a pin). New `unpinned_argv_tail_reads` walks the policy's JSON EST, so a comment is still not a read and a mixed permit is still flagged. Verify: `cargo test --lib cedar::engine` → 27 passed, incl. `a_positionally_anchored_argv_tail_permit_is_not_linted`; `cargo test --test policies` → 11 passed
