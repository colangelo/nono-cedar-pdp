...
- [ ] 5.1 Failing test: a daemon whose certificate is untrusted exits non-zero (T6)
- [ ] 5.2 Failing test: **nothing is ever accepted on the bind address** in that case — the window is the whole point, so assert the port is not listening, not merely that the process exited
- [ ] 5.3 Implement: throwaway listener on `127.0.0.1:0`, `rustls-platform-verifier` client, `ServerName` derived from `bind`, all **before** the real bind
