...
- ~~**#879's contents warrant a second look with authenticated access.**~~ **RESOLVED 2026-07-26** by reading the body directly (authenticated API). It is genuine and substantive — 210 lines by lukehinds, superseding #436 (an external request for an OPA-backed approval model). It names Cedar explicitly as an adapter category *"using principal/action/resource/context"*, and its boundary matches this project's thesis: *"Policy engines and SDKs may propose or approve permissions. nono remains responsible for validating and applying permissions."* No conflation with ContextForge/Microsoft abstractions. **Three corrections to how this project should read it:**
  1. It **explicitly defers Cedar** — Proposed First Milestone item 6: *"Defer Cedar, CUE, and Kyverno-specific adapters until the contract has stabilized,"* with OPA named as the first PoC.
  2. It describes a **different seam** from what this project implements: `PolicyInput → engine → PolicyDecision → nono validation → CapabilitySet → Landlock/Seatbelt`, evaluated at *sandbox-construction* time over a **stdin/stdout** adapter protocol (with a capability-*generation* mode). nono-cedar-pdp answers `WebhookApproval` over HTTP at *approval-escalation* time. Adjacent, overlapping in intent, **not the same contract** — this project must not claim to implement #879.
  3. **"Completed" shipped no code**: `PolicyInput` 0 hits, `CapabilityPlan` 0, `PolicyEngine` 0; the `PolicyDecision` hits are all the pre-existing native enum. The design direction is recorded, the contract is not.
